...
- [x] **Text logit equivalence** — max abs diff **1.06e-04** vs official HF Qwen3-0.6B (the M-RoPE swap preserves pretrained language ability)
- [x] **Vision tower equivalence** — **0.00e+00 (bit-exact)** hidden states vs official HF Qwen3-VL ViT
- [x] **KV-cache decode == full forward** (small random + real-weight multimodal)
- [x] **Full 849-page external benchmark reproduced** (KDoc-OCRBench-V2)

> Those gates run in the training repository's `tests/test_pierrotocrvlm.py --full`. This
> distribution carries the same model code with only the training paths removed.

---

## 🧠 How it works

The prompt string **is** the task switch ([tools/pierrotocr_common.py](tools/pierrotocr_common.py#L20-L39)).

| Prompt | Input | Output |
|---|---|---|
| `Layout Detection (fine):` | Downscaled page (1,024 tokens) | `class: x0,y0,x1,y1 ; …` — **listing order = reading order** (0–999 normalized coords) |
| `Layout Detection (coarse):` | ditto | DocLayNet convention (coarser grouping) |
| `Layout Detection (wild):` | Outdoor photo | Word-level `Text` boxes |
| `Text Recognition:` | Region crop (native resolution) | Plain text |
| `Table Recognition:` | Table crop | **OTSL** → restored to HTML at assembly |
| `Formula Recognition:` | Formula crop | LaTeX |
| `Page Recognition:` | Whole page | Page Markdown (single-pass reading) |

### Pipeline — coarse-to-fine, two passes

```
page image
   │
   ├─① layout pass    downscaled (~1,024 tokens) → "Layout Detection (fine):"
   │                   → [{label, box}] · listing order is reading order
   │
   ├─② recognition    crop from the ★native-resolution★ image using ①'s boxes →
   │                   ★batched generation★ per prompt class (Picture is skipped)
   │
   └─③ assembly       Markdown in reading order (tables=HTML, formulas=$$…$$) + JSON
```

- **Why crop from the original** — layout is read from a downscaled page, but the
  coordinates are 0–999 normalized so they apply directly to the original. Cropping the
  downscaled page would cut already-smeared small glyphs.
- **Why batch** — a page holds around 20 elements; one-by-one is slow. Same-prompt crops
  are grouped and generated at once with right padding + attention masks.

### Three modes

| Mode | Skeleton | When to use |
|---|---|---|
| `coarse-to-fine` (default) | Layout map | Documents where tables and reading order matter; coordinates come with it |
| `page` | Single-pass whole-page reading | Simple layouts; fast and strong on body text |
| `hybrid-page` | Whole-page reading + tables/headers grafted from c2f | Best benchmark path (pair with `--body-fill`) |

> **Band re-detection** (`--relayout-bands`) — on two-column documents the layout pass
> sometimes **skips an entire column**. It is not a threshold problem: the generation
> simply never emitted those boxes, so nothing can be recovered by relaxing a filter.
> Yet **hand the model just that region and it finds them exactly.** So we locate the
> empty slots geometrically and ask again for those parts only.

Same page, before the fix — the upper right column has no boxes at all (two paragraphs
vanished from the output):

![before band re-detection](docs/images/pierrotocrvlm/band-relayout/p02-before.jpg)

After band re-detection — boxes appear in the upper right column and the paragraphs are
back in place:

![after band re-detection](docs/images/pierrotocrvlm/band-relayout/p02-band-recovered.jpg)
