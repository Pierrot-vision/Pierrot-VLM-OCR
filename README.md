...
**PIERROT VLM · OCR** 은 **1인 VLM 연구·개발 프로젝트**이고, VLM 기반 이미지 문자 인식을
다룹니다. 이는 단순히 문자 인식뿐만 아니라 Document 블럭 감지, 일반 이미지의 감지를
포함합니다.

또한 **최소한의 크기로 최대 성능을 내는 것**이 목적입니다. 개발 환경부터 GPU 리소스가
넉넉하지 않았고, 실제로 돌릴 때의 비용도 아껴야 합니다. 무엇보다 **더 작은 모델이 더 큰
성능을 내는 것** 자체가 목표라, 이 프로젝트는 **0.986B** 라는 크기를 택했습니다 —
이 한 대가 레이아웃 검출부터 텍스트·표·수식 인식까지 전 과정을 처리합니다.
...
- 2026-09-01 — 🏆 **상용 1위 BizOnAI-OCR 과 3축 동률(76.90)** — **표는 58.32 로 역전**(+0.22p) · 머리말 **95.96**(+1.26p) · 본문 76.41(−1.49p). 0.99B 가 상용 모델과 같은 자리에 섰다. 채점은 **공백·머리글 규약 무시**이고 외부 모델엔 같은 보정을 걸 수 없다
- 2026-09-01 — 📈 **V3.8** — 공백무시 기준 3축 **73.74** (V3.7 73.48 대비 **+0.26p**) · 표 **48.84** · 머리말 **95.96**. 채택은 `final` 이 아니라 **`checkpoint-10500`** 이다
- 2026-08-28 — 📈 **V3.7** — 3축 평균 **73.48** 로 **자체 최고치** (V3.5 71.86 대비 **+1.62p**) · 표 **48.01** (+3.91p) · 머리말 **95.20** (+1.77p). 표에서 얻은 폭이 커 3축이 올랐다. 본문은 77.23 으로 1.31p 내려 BizOnAI(77.90) 아래가 됐다 — 종합 2위는 유지
- 2026-08-24 — 📈 **V3.6** — 3축 평균 **73.41** (V3.5 71.86 대비 **+1.55p**) · **세 축 모두 상승** — 표 **46.60** (+2.50p) · 머리말 **95.08** (+1.65p) · 본문(Long Text) **78.54** (+0.50p). 2위·본문 전체 1위는 그대로 유지
...
> 우리 값은 **공백·머리글 규약을 무시한** 채점, 외부 값은 **정식** 채점입니다.
> 외부 모델은 페이지별 예측물이 없어 같은 보정을 걸 수 없으므로 **같은 잣대가 아닙니다.**
...
```bash
# 벤치마크 최고점 경로(849쪽 3축 76.90 을 낸 조합)
