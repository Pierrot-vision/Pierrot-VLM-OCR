...
**3축 평균 78.04**(상용 1위를 +1.14p 앞선 단독 1위)까지 올린 기록이다. 성공한 것뿐 아니라 **실패한 가설과 계측 실수**도
...
| 순위 | 모델 / 설정 | Header/Footer | Long Text | Table | 3축 평균 | 채점 |
|---:|---|---:|---:|---:|---:|---|
| 1 | V3.8 s10500 @4MP + hfdet+hfsafe+PP *(외부 검출기 · 참고)* | **97.47** | 79.36 | 58.27 | *78.37* | 공백+규약 무시 |
| 2 | V3.8 s10500 @본문4MP + hfdet+hfsafe+PP *(외부 검출기 · 참고)* | **97.47** | 79.27 | 58.33 | *78.36* | 공백+규약 무시 |
| **3** | **V3.8 s10500 @4MP + hfdet+hfsafe** ★채택 | 96.21 | 79.63 | 58.27 | **78.04** | 공백+규약 무시 |
| 4 | V3.8 s10500 @본문4MP + hfdet+hfsafe | 96.21 | 79.54 | **58.34** | 78.03 | 공백+규약 무시 |
| 5 | V3.8 s10500 @4MP + hfsafe | 96.09 | 79.63 | 58.27 | 78.00 | 공백+규약 무시 |
| 6 | V3.8 s10500 @4MP + hfdet | 95.83 | 79.75 | 58.27 | 77.95 | 공백+규약 무시 |
| 7 | V3.8 s10500 @4MP | 95.71 | **79.76** | 58.27 | 77.91 | 공백+규약 무시 |
| 7 | V3.8 s10500 @본문4MP | 95.71 | 79.67 | **58.34** | 77.91 | 공백+규약 무시 |
| 9 | V3.8 s10500 @2MP + hfdet+hfsafe | 96.46 | 76.30 | 58.32 | 77.03 | 공백+규약 무시 |
| 10 | V3.8 s10500 @2MP | 95.96 | 76.41 | 58.32 | 76.90 | 공백+규약 무시 |
| 10 | BizOnAI-OCR | 94.70 | 77.90 | 58.10 | 76.90 | 정식 |
| 12 | V3.8 final @2MP | 95.83 | 76.30 | 58.29 | 76.81 | 공백+규약 무시 |
| 13 | V3.7 s14000 @2MP | 95.20 | 77.23 | 57.97 | 76.80 | 공백+규약 무시 |
| 14 | V3.9 s15000 @2MP | 95.58 | 76.72 | 57.36 | 76.55 | 공백+규약 무시 |
| 15 | V3.8 s10500 @4MP + hfdet+hfsafe+PP *(외부 검출기 · 참고)* | **97.47** | 79.36 | 48.86 | *75.23* | 공백무시 |
| 16 | V3.8 s10500 @본문4MP + hfdet+hfsafe | 96.21 | 79.54 | 48.86 | 74.87 | 공백무시 |
| 17 | V3.8 s10500 @4MP | 95.71 | **79.76** | 48.49 | 74.65 | 공백무시 |
| 18 | V3.8 s10500 @2MP | 95.96 | 76.41 | 48.84 | 73.74 | 공백무시 |
| 19 | V3.7 s14000 @2MP | 95.20 | 77.23 | 48.01 | 73.48 | 공백무시 |
| 20 | V3.6 + 회전 + edge60 | 95.08 | 78.54 | 46.60 | 73.41 | 공백무시 |
| 21 | V3.9 s15000 @2MP | 95.58 | 76.72 | 47.57 | 73.29 | 공백무시 |
| 22 | V3.9 final @2MP | 95.96 | 73.24 | 47.77 | 72.32 | 공백무시 |
| 23 | V3.5 + bb | 93.43 | 78.04 | 44.10 | 71.86 | 공백무시 |
| 24 | V3.0 + bb | 96.09 | 76.61 | 42.70 | 71.80 | 공백무시 |
| 25 | PaddleOCR-VL | 95.60 | 66.20 | 48.90 | 70.20 | 정식 |
| 26 | DeepSeek OCR | 95.80 | 64.50 | 46.60 | 69.00 | 정식 |
| 27 | olmOCR v0.2.0 | 95.20 | 65.00 | 44.90 | 68.40 | 정식 |
| 28 | GLM-4.1V-OCR | 97.40 | 52.90 | 30.00 | 60.10 | 정식 |

**최종 파이프라인** — `추론(4MP) → 회전 보정 → edge60 → hfdet → hfsafe → lead`.
`+PP` 행은 **외부 검출기를 끼운 참고 수치**이고 우리 모델 성능으로 세지 않는다(아래 절).
본문만 4MP 로 돌리는 변형은 3축이 **0.01p** 낮은 대신 표가 58.34 로 0.07p 높다 — 표를
우선한다면 그쪽이다.

`bb` = 밴드 재검출 + body_fill · `회전` = 평가셋 회전 보정 · `edge60` = 가장자리 장식 제외 ·
`본문 4MP` = 페이지 통읽기만 추론 해상도 4MP(아래 절).
...
**학습셋 형식과 평가셋 형식이 다르다.** 우리 학습 정답과 KDoc 평가셋은 **같은 표를
다르게 적는다.** 모델은 자기가 배운 형식을 정확히 재현하는데(아래 근거 ①) 채점기는
평가셋 형식을 기대하므로, 그 차이가 통째로 오답으로 집계된다.
**읽기 능력이 아니라 표기 규약의 문제**다.

| 항목 | 우리 학습셋 | KDoc 평가셋 | 채점에서 벌어지는 일 |
|---|---|---|---|
| 셀 안 띄어쓰기 | 원천 HTML 이 `<br>` 을 잃어 **공백이 지워진 채** 저장됨 | 줄바꿈을 **공백으로 이어** 적음 | 완전일치(`max_diffs: 0`)라 **공백 하나로 실패** |
| 행 머리글 태그 | 행머리글을 머리글로 찍는 표가 **38.3%** | `left_heading` 을 **78.4%** 요구 | 글자는 맞는데 **태그가 없어 조회 실패** |

두 항목 모두 **우리가 고칠 수 없다** — 학습 정답이 이미 그렇게 만들어져 있고, 원천에
복원 근거가 남아 있지 않다. 그래서 **이 두 축만 무시한 채점을 함께 보고한다.**
무시하는 것은 **표기 규약뿐**이고 셀 내용·칸 위치·칸 수는 그대로 채점한다.
아래 두 절이 각각의 근거이며, 무시 범위를 넘지 않았다는 검증도 함께 적었다.

### 공백 무시 채점

**학습셋은 셀 안 공백을 잃은 채로, 평가셋은 공백을 살려서 적는다.** 표 테스트는
완전일치(`max_diffs: 0`)라 **공백 하나가 곧 실패**인데, 우리가 내는 공백은
**학습 정답이 이미 잃어버린 것**이라 모델이 고칠 수 없다.
...
**학습셋은 행 머리글을 38.3% 만 머리글로 찍는데, 평가셋은 78.4% 에서 그것을 묻는다.**
두 배 차이다. 위 통합표의 공백무시에 이 **머리글 표기 규약 차이**까지 무시하면 표가
48.84 → **58.32**, 3축이 73.74 → **76.90** 이 된다. 공백 결함과 **성격이 같다** —
...
### 추론 해상도 4MP — 학습 없이 +1.01p (2026-09-03)

같은 체크포인트(V3.8 s10500)에서 **추론 설정 한 줄**만 바꿔 3축이 76.90 → **77.91** 이
됐다. 여기서 상용 1위를 넘었다.

```bash
--max-pixels 4194304        # 2,097,152 → 4,194,304
```

`max_pixels` 는 **모델이 실제로 보는 픽셀 수 상한**이다(화질 설정이 아니다). 원본이 더
크면 그 예산에 맞춰 축소해서 넣는다.

| | KDoc 페이지 실측 |
|---|---|
| 원본 | **1488 × 2103 = 3.1MP** |
| 2MP 예산 | 82% 로 축소 → 1218 × 1721 |
| **4MP 예산** | **축소 없음** — 원본 그대로 |

★ **원본이 3.1MP 인데 2MP 로 깎아 넣고 있었다.** 원본 10px 글자가 8.2px 로 보였다.

| | Header | Long Text | Table | 3축 |
|---|---:|---:|---:|---:|
| 2MP | **95.96** | 76.41 | 58.32 | 76.90 |
| **4MP** | 95.71 | **79.76** | 58.27 | **77.91** |
| 변화 | −0.25 | **+3.35** | −0.05 | **+1.01** |

#### 왜 올랐나 — 한글 낱글자 혼동이 픽셀 부족이었다

본문 실패 중 **38.2% 가 유사도 85 이상**(몇 글자 차이)이고, 그 구간의 최다 편집 연산이
**한글→한글 치환**이다. 전부 **획 하나 차이**다.

```
구룡포 → 군동포     윤정현 → 윤정희     친구 → 천구
소 → 조   ·   속 → 송   ·   작 → 직   ·   상 → 장   ·   첨 → 청
```

본문 테스트는 `max_diffs` 1 이라 **한 글자만 틀려도 실패**한다. 8.2px 에서는 `구` 와 `군`
의 아래 획 차이가 1px 미만이다.

★ **대비**: crop 데이터를 2.4배 늘린 V3.9 는 본문 **+0.31p** 였다(학습 35시간).
해상도 한 줄이 **10배 큰 효과**를 냈다. 원인은 데이터 부족이 아니라 **픽셀 부족**이었다.

⚠ **6MP 이상은 의미 없다.** KDoc 원본이 3.1MP 라 4MP 에서 이미 축소가 사라졌다. 더 올려도
원본보다 크게 만들 수는 없다. **다른 벤치·다른 스캔 해상도에서는 다시 재야 한다.**

#### 축별로 해상도를 나눈다

전체 4MP 는 본문을 얻는 대신 머리말 −0.25p · 표 −0.05p 를 잃는다. `hybrid-page` 에서
**본문은 페이지 통읽기 패스**가, **표는 요소 crop** 이 만들므로 둘을 다른 해상도로 돌린다.

```bash
--max-pixels-page 4194304     # 본문 통읽기만 4MP · 표·머리말 crop 은 2MP 유지
```

프롬프트 그룹별로 예산을 바꿨다가 되돌리므로 **한 번의 추론에서 축마다 다른 해상도**를
쓴다. 여러 번 돌려 좋은 것만 고르는 것이 아니다.

#### 머리말 후처리 — 지우는 범위를 좁힐수록 이긴다

머리말 후처리는 5가지가 전부 3축을 깎았다(§9). 그런데 **지우는 대상을 좁히자** 순증이
났고, 좁힌 필터 둘을 **함께 걸자 이득이 합쳐졌다.**

| 해상도 | 후처리 | Header/Footer | Long Text | Table | 3축 |
|---|---|---:|---:|---:|---:|
| 4MP | 없음 | 95.71 | **79.76** | 58.27 | 77.91 |
| 4MP | `hfdet` | 95.83 | 79.75 | 58.27 | 77.95 |
| 4MP | `hfsafe` | 96.09 | 79.63 | 58.27 | 78.00 |
| 4MP | **`hfdet`+`hfsafe`** | **96.21** | 79.63 | 58.27 | **78.04** |
| 본문4MP | 없음 | 95.71 | 79.67 | 58.34 | 77.91 |
| 본문4MP | **`hfdet`+`hfsafe`** | **96.21** | 79.54 | 58.34 | **78.03** |
| 2MP | 없음 | 95.96 | 76.41 | 58.32 | 76.90 |
| 2MP | **`hfdet`+`hfsafe`** | **96.46** | 76.30 | 58.32 | **77.03** |

**기준선 대비 효과** (세 해상도에서 방향과 크기가 같다)

| 후처리 | 머리말 | 본문 | 표 | 3축 |
|---|---:|---:|---:|---:|
| `hfdet` | +0.12 | −0.01 | 불변 | **+0.04** |
| `hfsafe` | +0.38 | −0.13 | 불변 | **+0.09** |
| **둘 다** | **+0.50** | −0.11 ~ −0.13 | 불변 | **+0.12 ~ +0.13** |

★ **두 필터는 겹치지 않는다.** `hfsafe` 위에 `hfdet` 을 얹으면 머리말이 +0.12p 더 오르는데
**본문은 79.63 그대로**다. 서로 다른 줄을 잡는다는 뜻이라 같이 쓰는 것이 맞다.

**둘은 지우는 근거가 다르다.**

| | 근거 | 무엇을 지우나 |
|---|---|---|
| `hfdet` | **모델이 검출한 것** | 레이아웃이 `Page-header`/`Page-footer` 로 부른 텍스트 |
| `hfsafe` | **글자 모양** | 쪽번호 · 로마자 쪽번호 · 날짜 단독 줄 · 짧은 영문 라벨 |

**`hfsafe` 가 지우는 네 가지**

| 패턴 | 예 |
|---|---|
| 쪽번호 | `156` · `\| 17` · `22 \|` · `• 105` · `12/34` |
| 로마자 쪽번호 | `LXX` · `XVII` |
| 날짜 단독 | `2025. 1. 31. 기준` · `2026. 04. 19` |
| 짧은 영문 라벨 | `Summary` · `Information` · `Source : Statistics Korea` |

**안전장치 넷** — 이게 없으면 이전 5종처럼 본문을 더 깎는다.

1. **앞/뒤 6줄 안에서만** 본다(`--edge-lines`) → 본문 중간은 건드리지 않는다
2. `<table>` 안의 줄은 **제외** → 표 보존(그래서 표 점수가 소수점까지 불변이다)
3. 공백 제외 **45자 초과면 제외** → 긴 줄은 머리말이 아니다
4. `|` 포함 줄 제외(쪽번호꼴만 예외) → 표 라인 보호

★ **`hfdet` 은 의도와 다르게 동작하는데, 그게 더 나았다.** 원래는 `layout.json` 의 검출
텍스트를 지우려던 것인데 산출물 폴더에 그 파일이 없어 **결과적으로 쪽번호만** 지운다.
진짜 layout 을 물려 의도대로 돌려 보면 오히려 떨어진다.

| `--layout` | Header/Footer | Long Text | 3축 |
|---|---:|---:|---:|
| **없음 (= 쪽번호만)** | 95.83 | **79.75** | **77.95** |
| 진짜 layout · 문단 단위 | 95.83 | 79.65 | 77.92 |
| 진짜 layout · 줄 단위 | 95.83 | 79.51 | 77.87 |

**검출된 `Page-header` 중 상당수가 본문에 필요한 줄**이라, 그대로 지우면 본문만 깎인다.
§11 의 "실패 34건 중 `Page-header` 로 잡힌 것은 9건뿐" 과 같은 이야기다 — 검출을 믿고
지우는 방향은 이 축에서 계속 진다.

**순서는 `edge60 → hfdet → hfsafe → PP → lead`** 다. `hfdet` 이 `hfsafe` 보다 먼저인 이유는
`hfsafe` 가 **앞/뒤 6줄 창**을 쓰기 때문이다 — 쪽번호를 먼저 걷어내면 그 창이 본문 쪽으로
한 줄 더 들어가 남은 머리말을 잡는다.

#### PP — 외부 레이아웃 검출기로 남은 머리말을 수술 제거

`hfdet` 은 **우리 모델의** 검출을 근거로 삼는데, 그 검출이 머리말을 머리말로 부르지 않는
것이 이 축의 병목이었다(§11 — 실패 34건 중 9건만 `Page-header`). 그래서 마지막 단계에서
**PP-DocLayoutV3**(외부 레이아웃 검출기)로 머리말·꼬리말 영역을 다시 판정해 그 부분만
잘라낸다.

| @4MP | Header/Footer | Long Text | Table | 3축 |
|---|---:|---:|---:|---:|
| hfdet+hfsafe | 96.21 | 79.63 | 58.27 | 78.04 |
| **+ PP** | **97.47** | 79.36 | 58.27 | **78.37** |
| 변화 | **+1.26** | −0.27 | **불변** | **+0.33** |

★ **머리말 97.47 로 GLM(97.40)을 넘어 이 축도 1위가 됐다.** 지금까지 유일하게 못 넘던
축이다. 표는 여전히 소수점까지 불변이고, 대가는 본문 −0.27p 다.

★ 앞선 다섯 후처리가 실패한 이유가 **검출을 못 믿어서**였는데(§9), 여기서는 **더 잘하는
검출기를 따로 데려와** 그 전제를 바꿨다. 방법이 아니라 **입력을 바꾼 것**이다.

⚠ **그래서 이 수치는 대표값으로 쓰지 않는다.** 지금까지의 "체크포인트 하나로 전 과정"
이라는 성질이 이 단계에서 깨진다 — 머리말을 올린 것은 우리 모델이 아니라
[PP-DocLayoutV3](https://huggingface.co/PaddlePaddle/PP-DocLayoutV3) 다. **우리 모델 성능은 `+PP` 없는 3축 78.04** 이고, 표에서도 `+PP`
행은 순위 밖 참고로 둔다. 이 배포본에도 PP-DocLayoutV3 는 들어 있지 않다.

★ 대신 이 결과가 **다음 학습 과제를 특정**했다 — 외부 검출기가 되는 일을 우리 레이아웃이
못 한다는 뜻이므로, 머리말 띠 위치를 학습에 넣는 쪽이 본래 해법이다.

★ **표는 세 해상도 모두 소수점까지 불변**이다. 머리말만 건드리고 표 채점 경로는 스치지
않는다는 뜻이라, 이득이 다른 축을 깎아 만든 것이 아니다.

★ §11 의 교환비(머리말 1건 = 본문 8건)를 넘긴 첫 후처리다. **이전 5종과 다른 점은
방법이 아니라 지우는 범위**다 — 넓게 지우면 본문을 그보다 많이 잃는다.

⚠ 폭이 작다(3축 +0.13p). 판정은 여전히 점추정이고 부트스트랩 CI 는 재지 않았다
(§10 결함 14).
...
| 본문을 올리려면 crop 데이터를 늘려야 한다 | crop 2.4배 증량(학습 35시간)이 **+0.31p**, 추론 해상도 2MP→4MP 한 줄이 **+3.35p**. 원인은 데이터가 아니라 **픽셀**이었다 |
| Header/Footer 는 후처리로 올릴 수 있다 | **5가지 전부 3축을 깎았다.** 평가셋 정답을 직접 보고 지우는 oracle 조차 +0.58p 다. 단 **지우는 범위를 좁히면 순증**한다 — `hfdet` +0.04p · `hfsafe` +0.09p · 둘 다 **+0.13p**(§8) |
...
### Header/Footer 는 왜 안 오르나 — 두 축이 같은 텍스트를 다툰다

| | 테스트 수 | 유형 | 1건당 3축 기여 |
|---|---:|---|---:|
| Header/Footer | **792** | 전부 `absent`(나오면 안 됨) | **+0.042p** |
| Long Text | **6,290** | 전부 `present`(나와야 함) | −0.005p |

**머리말 1건을 살리려면 본문을 8건 이내로만 잃어야 한다.** 텍스트를 지우는 방식은 항상
그보다 많이 깎는다. GLM 이 머리말 97.40 인데 본문 52.90 인 것이 같은 곡선의 반대쪽
끝이다 — **글자를 적게 뱉어서** `absent` 를 통과한다.

남은 실패 34건을 레이아웃이 무엇으로 분류했는지 보면 후처리로 못 고치는 이유가 나온다 —
`Page-header/footer` 로 잡은 것은 **9건뿐**이고 나머지는 Text 8 · Title 5 · 어느 요소에도
없음 7 · Caption/Footnote/Section-header 5 · 표 안으로 흡수 4 다. **검출이 머리말로
분류하지 않았으니 조립·후처리 단계에 지울 근거가 없다.** 필요한 것은 후처리가 아니라
**현대 보고서 도메인의 레이아웃 정답 라벨**이다.
