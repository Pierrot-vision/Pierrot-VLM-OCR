...
**3축 평균 78.37**(상용 1위를 +1.47p 앞선 단독 1위)까지 올린 기록이다. 성공한 것뿐 아니라 **실패한 가설과 계측 실수**도
...
|---:|---|---:|---:|---:|---:|---|
| **1** | **V3.8 s10500 @4MP + hfdet+hfsafe+PP** ★채택 | **97.47** | 79.36 | 58.27 | **78.37** | 공백+규약 무시 |
| 2 | V3.8 s10500 @본문4MP + hfdet+hfsafe+PP | **97.47** | 79.27 | 58.33 | 78.36 | 공백+규약 무시 |
| 3 | V3.8 s10500 @4MP + hfdet+hfsafe | 96.21 | 79.63 | 58.27 | 78.04 | 공백+규약 무시 |
...
| 15 | V3.8 s10500 @4MP + hfdet+hfsafe+PP | **97.47** | 79.36 | 48.86 | 75.23 | 공백무시 |
...
**최종 파이프라인** — `추론(4MP) → 회전 보정 → edge60 → hfdet → hfsafe → PP 수술 제거 → lead`.
...
⚠ **대가: 파이프라인에 외부 모델이 하나 들어온다.** 지금까지의 "체크포인트 하나로 전 과정"
이라는 성질이 이 단계에서만 깨진다. 이 배포본에는 PP-DocLayoutV3 가 들어 있지 않으므로
`+PP` 없이 돌리면 3축 **78.04** 가 기준이다.
