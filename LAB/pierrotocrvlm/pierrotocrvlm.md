...
[10 계측 결함 13건](#10-계측-결함-13건) · [11 남은 병목](#11-남은-병목)
...
| **2** | **V3.8 s10500** ★채택 | 95.96 | 76.41 | 48.84 | **73.74** | 공백무시 |
| 3 | V3.8 final | 95.83 | 76.30 | 48.51 | 73.55 | 공백무시 |
| 4 | V3.7 s14000 | 95.20 | 77.23 | 48.01 | 73.48 | 공백무시 |
| 5 | V3.6 + 회전 + edge60 | 95.08 | **78.54** | 46.60 | 73.41 | 공백무시 |
| 6 | V3.8 s10000 | 95.71 | 75.60 | 47.03 | 72.78 | 공백무시 |
| 7 | V3.5 + 밴드 + body_fill | 93.43 | 78.04 | 44.10 | 71.86 | 공백무시 |
| 8 | V3.0 + 밴드 + body_fill | 96.09 | 76.61 | 42.70 | 71.80 | 공백무시 |
| 9 | V3.0 + body_fill | 96.72 | 74.74 | 42.42 | 71.29 | 공백무시 |
| 10 | PaddleOCR-VL | 95.60 | 66.20 | 48.90 | 70.20 | 정식 |
| 11 | DeepSeek OCR | 95.80 | 64.50 | 46.60 | 69.00 | 정식 |
| 12 | V3.4t s1500 | 95.83 | 64.15 | 45.66 | 68.55 | 공백무시 |
| 13 | V3.4t s500 | 96.72 | 64.88 | 43.98 | 68.52 | 공백무시 |
| 14 | olmOCR v0.2.0 | 95.20 | 65.00 | 44.90 | 68.40 | 정식 |
| 15 | V3.5 final (조립 없음) | 93.94 | 64.67 | 44.07 | 67.56 | 공백무시 |
| 16 | V3.0 (조립 없음) | 96.97 | 62.93 | 42.42 | 67.44 | 공백무시 |
| 17 | GLM-4.1V-OCR | **97.40** | 52.90 | 30.00 | 60.10 | 정식 |
...
**공백무시를 기본으로 쓰는 이유**: 학습셋 원천 HTML 이 셀 안 줄바꿈을 구분자 없이
이어붙여 놔서, strict 채점은 **우리가 고칠 수 없는 띄어쓰기**를 오류로 센다(표 15~17p 손실).
원천 HTML 에서 원래 공백을 복원하는 것은 근거가 없어 **불가능**하다.

★ **표가 48.84 까지 올라오면서 3축 1위(76.90)와의 격차가 3.16p 로 좁혀졌다.**
V3.0 의 42.42 에서 **+6.42p** 다. 다만 표는 아직 PaddleOCR-VL(48.90)과 사실상 동률이고
BizOnAI(58.10)와는 **9.26p** 남았다 — 여전히 표가 마지막 축이다.

★ **V3.7 부터는 표를 얻고 본문을 내주는 교환이다.** 본문 최고치는 V3.6 의 **78.54**(리더보드
전체 1위였다)인데, V3.7 77.23 → V3.8 **76.41** 로 두 판 연속 내렸다. 3축 평균은 올랐지만
본문 단독으로는 BizOnAI(77.90) 아래로 내려갔다. **어느 축을 대표로 볼지에 따라 최고 모델이
갈린다** — 3축이면 V3.8, 본문이면 V3.6 이다.

★ **채택은 `final` 이 아니라 `checkpoint-10500` 이다.** V3.8 은 s10000 72.78 → **s10500
73.74** → final 73.55 로 중간에서 꺾인다. V3.4t 에서 확인한 "eval loss 최저점 ≠ 벤치
최고점"(§5)이 다시 재현됐다.
...
V3.5 배합의 이득은 대부분 **조립과 맞물렸을 때** 나온다. (V3.6 의 조립 없는 값은 미측정이다.)

### 머리글 규약까지 무시한 채점 — 상용 1위와 동률 (2026-09-01)

위 표는 **공백무시**다. 여기에 **머리글(header) 표기 규약 차이**까지 무시하면 그림이 또
달라진다. 표 점수가 48.84 → **58.32** 로 뛰는데, 이 폭이 곧 **우리 출력 규약과 채점기
기대치의 간극**이지 읽기 능력의 차이가 아니라는 뜻이다(§3 의 `<ched>` 사건과 같은 계열).
...
| 8 | PaddleOCR-VL | 95.60 | 66.20 | 48.90 | 70.20 | 정식 |
| 9 | DeepSeek OCR | 95.80 | 64.50 | 46.60 | 69.00 | 정식 |
| 10 | olmOCR v0.2.0 | 95.20 | 65.00 | 44.90 | 68.40 | 정식 |
| 11 | GLM-4.1V-OCR | **97.40** | 52.90 | 30.00 | 60.10 | 정식 |

**0.99B 가 상용 1위와 3축 동률(76.90)이고, 축별로는 2승 1패다** — 표 +0.22p · 머리말
+1.26p · 본문 −1.49p.

⚠ **그래도 같은 잣대가 아니다.** 외부 모델은 **페이지별 예측물을 갖고 있지 않아** 같은
보정을 걸 수 없다. 공백무시 때와 똑같은 제약이며, 보정을 건 쪽만 이득을 본다.
"우리 규약으로 채점하면 동률" 이지 "정식 채점에서 동률" 이 아니다.

★ 그럼에도 읽어야 할 신호는 **표 58.32** 다. §8 내내 "남은 격차는 표 하나" 였는데,
그 격차의 상당 부분이 **능력이 아니라 출력 규약**이었음을 이 수치가 보여 준다.
정식 채점에서 이걸 회수하려면 **모델이 채점기가 기대하는 머리글 표기를 직접 내도록**
GT 규약을 다시 맞춰야 한다(후처리로 덮으면 §6 의 "지표 화장" 이다).
...
★ 위 값들은 **주변 비율**이라 곱하면 안 된다. 헤더 정확도는 **셀값·위치가 모두 성공한
테스트를 분모로** 조건부 측정해야 upstream 누락이 헤더 실패로 중복 계산되지 않는다.

확정된 별개 사실 둘: **학습·추론 길이 불일치**(학습 상한 2,900 / 추론 4,096, 표 crop
13.3% 가 상한 도달)와 **표 crop 격자 붕괴**(6.7%, 86건 중 85건이 `<nl>` 을 한 번도 내지
않는다 — "읽다가 무너지는" 게 아니라 처음부터 행 구분을 못 한다. 다만 재현율이 36% 라
**원인 미확정**이며, 확정 전까지 표 데이터 방향은 전부 미확정으로 둔다).

본문 쪽 남은 지렛대는 전부 1~2p 대다.
...
→ **Long Text 전용 학습셋은 추가하지 않는다.** 데이터 한 종류로 잡히는 형태가 아니다.
