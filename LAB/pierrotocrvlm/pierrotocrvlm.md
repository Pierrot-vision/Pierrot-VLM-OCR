...
**3축 평균 71.86** 까지 올린 기록이다. 성공한 것뿐 아니라 **실패한 가설과 계측 실수**도
...
**기본 알고리즘은 MinerU2.5 다.** 단일 체크포인트가 프롬프트만 바꿔 레이아웃 검출과
영역 인식을 모두 수행하는 coarse-to-fine 2단계 골격, 좌표를 텍스트로 생성하는
detection-as-text, 표는 OTSL — 이 설계를 그대로 따랐다.

바꾼 것은 두 가지뿐이다.

1. **부품 교체** — Qwen2-VL ViT + Qwen2-0.5B → **Qwen3-VL-2B ViT(+DeepStack) + Qwen3-0.6B**
...
뿌리가 넷이다 — 뼈대, 문서를 볼 수 있게 하는 장치, 태스크 설계, 데이터·보상.
...
**한 줄로: LLaVA = 뼈대, Qwen-VL = 문서를 볼 수 있게 하는 장치, MinerU = 태스크 설계.**
...
1. **작아진 ViT 보완** — 675M·32층을 306M·24층으로 줄였으므로 마지막층만 쓰면 작은 글자·
...
⚠ **미해결 트레이드오프.** MinerU 는 레이아웃만 고정 정사각을 택했다 — 격자가 항상
37×37 이면 "격자 위치↔좌표" 대응을 한 번만 배우면 되므로 **저데이터에서 유리**하다.
우리가 가변 격자를 유지하는 근거는 M-RoPE 가 토큰별 (h,w) 를 명시 인코딩하고 타깃
좌표가 원본 기준 0~999 라 격자와 독립이라는 점인데, **레이아웃 gold 가 MinerU 의 1/128**
...
loss 는 토큰 평균이므로 모델이 실제로 받는 표의 몫은 47.4% 다. 반대로 **crop 은 레코드를
3배 늘려도 글자 몫이 2.8% → 5.5% 에 그친다** — "crop 을 늘렸는데 표가 안 움직인다"의
정체가 이것이다. 배합표에는 반드시 두 축을 함께 적는다.
...
**결과부터: V3.5 가 현재 최고 모델이다** — 밴드 + body_fill 조립과 함께 3축 **71.86**,
본문 **78.04 로 리더보드 전체 1위**(BizOnAI 77.90). 표도 42.70 → **44.10** 으로 올랐고,
대가는 머리말 96.09 → **93.43**(−2.66p)이다. 다만 **조립을 끄면 67.56 으로 V3.0(67.44)과
사실상 같다**(+0.12p) — 배합의 이득이 조립과 맞물려서만 드러난다(§8).
...
| **2** | **V3.5 + 밴드 + body_fill** | 93.43 | **78.04** | 44.10 | **71.86** | 공백무시 |
| 3 | V3.0 + 밴드 + body_fill | 96.09 | 76.61 | 42.70 | 71.80 | 공백무시 |
| 4 | V3.0 + body_fill | 96.72 | 74.74 | 42.42 | 71.29 | 공백무시 |
| 5 | PaddleOCR-VL | 95.60 | 66.20 | 48.90 | 70.20 | 정식 |
| 6 | DeepSeek OCR | 95.80 | 64.50 | 46.60 | 69.00 | 정식 |
| 7 | V3.4t s1500 | 95.83 | 64.15 | 45.66 | 68.55 | 공백무시 |
| 8 | V3.4t s500 | 96.72 | 64.88 | 43.98 | 68.52 | 공백무시 |
| 9 | olmOCR v0.2.0 | 95.20 | 65.00 | 44.90 | 68.40 | 정식 |
| 10 | V3.5 final (조립 없음) | 93.94 | 64.67 | 44.07 | 67.56 | 공백무시 |
| 11 | V3.0 (조립 없음) | 96.97 | 62.93 | 42.42 | 67.44 | 공백무시 |
| 12 | GLM-4.1V-OCR | **97.40** | 52.90 | 30.00 | 60.10 | 정식 |

⚠ **직접 비교가 아니다.** 우리 값은 **공백무시**, 외부는 **정식** 채점이라 우리 쪽이
유리한 자다(50쪽 실측: PaddleOCR-VL 정식 53.71 → 공백무시 59.11).
정식 기준으로도 3축 61.04 → **64.86** 이라 olmOCR·DeepSeek 사거리에는 들어와 있다.
...
★ **본문(Long Text)은 78.04 로 처음 1위가 됐다**(BizOnAI 77.90). 남은 격차는
**표 하나**다(44.10 vs 58.10).
...
V3.5 배합의 이득은 대부분 **조립과 맞물렸을 때** 나온다(본문 76.61 → 78.04, 표 42.70 → 44.10).

★ **머리말은 대가를 치렀다.** V3.5 는 93.43 으로 V3.0(96.09)보다 2.66p 낮다.
3축 평균은 올랐지만 이 축만 단독으로 보면 회귀다.
