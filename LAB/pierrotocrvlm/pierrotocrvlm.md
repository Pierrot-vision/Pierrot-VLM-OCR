...
아래는 한국어 실표 학습셋에서 그대로 뽑은 셀이다.

```
✗ 사업이해및실무(7)              ✗ 질병관리청만성질환예방과
✗ 식품알레르기와아나필락시스      ✗ 한국천식알레르기협회추천
✗ 측정방법(또는측정산식)         ✗ 부산아토피천식교육정보센터(팀장)
✗ 교육내용및 교과목특이사항       ✗ 소속및 직위

✓ -602동1~11라인 1403호,1302호,1105호     ← 같은 파일에도 정상 셀은 있다
✓ -603동1~8라인 10층1본,9층1본,2층1본
```

**12자 이상인데 공백이 하나도 없는 셀이 6.8%** 다. `<br>` 을 텍스트 없는 태그로
흘려버린 변환의 흔적이고, 원천 HTML 에는 그 자리에 무엇이 있었는지가 **남아 있지
않다** — 그래서 복원은 근거가 없어 **불가능**하다.
...
#### 무엇이 어긋나는가 — 정답 OTSL 과 나란히 놓고 본다

KDoc 은 정답 표를 공개하지 않고 테스트만 준다. 그래서 **정답 OTSL 을 가진 한국어 실표
val** 에서 원본 이미지·정답·우리 출력을 나란히 놓았다(V3.8 s10500).
아래 숫자는 태그별 `정답 / 예측` 개수다.
