...
표 테스트는 완전일치(`max_diffs: 0`)라 **공백 하나가 곧 실패**다.
...
원인은 모델이 아니라 **학습 정답 자체**다. 같은 병이 GT 에 그대로 있다 —
`<fcel>교육내용및 교과목특이사항` · `<fcel>소속및 직위` 처럼 **12자 이상인데 공백이
하나도 없는 셀이 6.8%** 다. `<br>` 을 텍스트 없는 태그로 흘려버린 변환의 흔적이고,
원천 HTML 에는 그 자리에 무엇이 있었는지가 **남아 있지 않다** — 그래서 복원은
근거가 없어 **불가능**하다.

⚠ 정규화로 덮는 것도 복원이 아니다. `의 료 기 관 → 의료기관` 같은 삽입 공백 제거에는
맞지만 **정상 띄어쓰기까지 붙여 버린다.** 그래서 정규화본은 전량 투입하지 않고
소량 A/B 물량으로만 썼다.
...
#### 무엇이 어긋나는가 — 정답 OTSL 과 태그 대조

KDoc 은 정답 표를 공개하지 않고 테스트만 준다. 그래서 **정답 OTSL 을 가진 한국어 실표
val** 로 태그 수를 직접 세어 봤다(V3.8 s10500).

| 사례 | | `<ched>` | `<fcel>` | `<ecel>` | `<lcel>` | `<ucel>` | `<nl>` |
|---|---|---:|---:|---:|---:|---:|---:|
| **1** (태그 절대차 65) | 정답 | 13 | 37 | 11 | 0 | **27** | 22 |
| | 예측 | 12 | 22 | **2** | 0 | **0** | **9** |
| **2** (절대차 44) | 정답 | 11 | 38 | 15 | 2 | 0 | 11 |
| | 예측 | 13 | 17 | **4** | **7** | 0 | **6** |
| **3** (절대차 0) | 정답 = 예측 | 17 | 48 | 0 | 4 | 1 | 10 |

★ **글자를 못 읽어서가 아니다. 빈 칸과 병합을 덜 낸다.** 실패한 두 사례 모두
`<ecel>`(빈 칸)과 `<ucel>`(세로 병합)이 정답보다 훨씬 적고, `<nl>`(행 끝)이 절반 수준이다.
**빈 칸 하나를 빠뜨리면 그 뒤 칸이 전부 한 칸씩 밀린다** — 셀 내용은 맞는데 좌표가
어긋나는 "관계 불일치 35.24%" 와 "열 밀림 48.1%" 의 실체가 이것이다.

★ 그리고 **사례 3 처럼 태그가 완전히 일치하는 표도 있다.** 능력이 없는 게 아니라
특정 구조(빈 칸이 많고 세로 병합이 깊은 표)에서 무너진다.

(대조 뷰어 `results/viewer/gt_vs_pred.html` — 원본 이미지·정답·예측을 나란히 렌더한다.
학습 저장소 산출물이라 이 배포본에는 없다.)
