...
**3축 평균 71.80** 까지 올린 기록이다. 성공한 것뿐 아니라 **실패한 가설과 계측 실수**도
...
| | PierrotOCRVLM | MinerU2.5-Pro |
|---|---|---|
| 전체 | **0.986B** | 1.16B (표기 1.2B) |
| 눈 | 306M / 24층 / h1024 | 631M / 32층 / h1280 |
| 뇌 | **596M / 28층 / h1024** | 494M / 24층 / h896 |
| 패치 / 유효단위 | 16 / **32px** | 14 / 28px |
| 레이아웃 입력 | 종횡비 유지, ≤1024² 예산 | **1036² 고정** (=37×37, 토큰 1,369 고정) |
| ViT 중간층 | **DeepStack 5/11/17** | 안 씀(최종층만) |
| 커넥터 | 머저 4개 | PatchMerger 1개 |
| 위치 인코딩 | interleaved M-RoPE | sectioned M-RoPE [8,12,12] |
| 스크래치 비율 | 83.9M = **8.5%** | 31M = 2.7% |

**눈을 줄이고 뇌를 키운 배분**이다. MinerU 의 1036 은 28 의 배수라서 나온 값이고,
우리는 32 배수 격자라 그대로 못 쓴다.

### 2.3 계보별 구조 비교 — LLaVA 에서 우리까지
...
| **2** | **V3 + body_fill + 밴드** | 96.09 | **76.61** | 42.70 | **71.80** | 공백무시 |
| 3 | V3 + body_fill | 96.72 | 74.74 | 42.42 | 71.29 | 공백무시 |
| 4 | PaddleOCR-VL | 95.6 | 66.2 | 48.9 | 70.2 | 정식 |
| 5 | DeepSeek OCR | 95.8 | 64.5 | 46.6 | 69.0 | 정식 |
| 6 | V3.4t s1500 | 95.83 | 64.15 | 45.66 | 68.55 | 공백무시 |
| 7 | olmOCR v0.2.0 | 95.2 | 65.0 | 44.9 | 68.4 | 정식 |
| 8 | V3 (기준선) | 96.97 | 62.93 | 42.42 | 67.44 | 공백무시 |
| 9 | GLM-4.1V-OCR | **97.4** | 52.9 | 30.0 | 60.1 | 정식 |
...
★ **본문은 1위와 1.3p 차까지 좁혔고, 남은 격차는 표 하나다**(42.70 vs 58.1).
그리고 기준선 67.44 → 71.80 (**+4.36p**)는 **재학습이 아니라 추론·조립 경로 수정만으로**
얻은 것이다.
