...
1. **부품 구성** — **Qwen3-VL-2B ViT(+DeepStack) + Qwen3-0.6B**. 선행 구성보다 **눈을 줄이고 뇌를 키웠다**(§2.3)
...
2. **언어 hidden 차원으로 투영한다** — 이식 원본의 출력은 2048(그쪽 언어 폭)인데 우리
   디코더는 1024 다. 이 한 줄 때문에 사전학습 머저를 그대로 쓸 수 없다.
...
`view` 한 번으로 이웃이 접히는 것은 패치 시퀀스가 애초에 **머저 블록 우선 순서**로
나오기 때문이다 — 별도 permute 가 필요 없다.
...
ablation(A 없음 / B 1개 / C 3개 / D gated)은 **아직 돌리지 않았다.**
