...
**3축 평균 77.94**(상용 1위를 +1.04p 앞선 단독 1위)까지 올린 기록이다. 성공한 것뿐 아니라 **실패한 가설과 계측 실수**도
...
|---:|---|---:|---:|---:|---:|---|
| 1 | V3.8 s10500 @4MP + 안전HF | 95.83 | **79.75** | 58.27 | **77.95** | 공백+규약 무시 |
| **2** | **V3.8 s10500 @본문4MP + 안전HF** ★채택 | 95.83 | 79.65 | **58.34** | **77.94** | 공백+규약 무시 |
| 3 | V3.8 s10500 @4MP | 95.71 | **79.76** | 58.27 | 77.91 | 공백+규약 무시 |
| 3 | V3.8 s10500 @본문4MP | 95.71 | 79.67 | **58.34** | 77.91 | 공백+규약 무시 |
| 5 | V3.8 s10500 @2MP + 안전HF | **96.09** | 76.39 | 58.32 | 76.93 | 공백+규약 무시 |
| 6 | V3.8 s10500 @2MP | 95.96 | 76.41 | 58.32 | 76.90 | 공백+규약 무시 |
| 6 | BizOnAI-OCR | 94.70 | 77.90 | 58.10 | 76.90 | 정식 |
| 8 | V3.8 final @2MP | 95.83 | 76.30 | 58.29 | 76.81 | 공백+규약 무시 |
| 9 | V3.7 s14000 @2MP | 95.20 | 77.23 | 57.97 | 76.80 | 공백+규약 무시 |
| 10 | V3.9 s15000 @2MP | 95.58 | 76.72 | 57.36 | 76.55 | 공백+규약 무시 |
| 11 | V3.8 s10500 @본문4MP + 안전HF | 95.83 | 79.65 | 48.86 | 74.78 | 공백무시 |
| 12 | V3.8 s10500 @4MP + 안전HF | 95.83 | 79.75 | 48.49 | 74.69 | 공백무시 |
| 13 | V3.8 s10500 @2MP | 95.96 | 76.41 | 48.84 | 73.74 | 공백무시 |
| 14 | V3.7 s14000 @2MP | 95.20 | 77.23 | 48.01 | 73.48 | 공백무시 |
| 15 | V3.6 + 회전 + edge60 | 95.08 | 78.54 | 46.60 | 73.41 | 공백무시 |
| 16 | V3.9 s15000 @2MP | 95.58 | 76.72 | 47.57 | 73.29 | 공백무시 |
| 17 | V3.9 final @2MP | 95.96 | 73.24 | 47.77 | 72.32 | 공백무시 |
| 18 | V3.5 + bb | 93.43 | 78.04 | 44.10 | 71.86 | 공백무시 |
| 19 | V3.0 + bb | 96.09 | 76.61 | 42.70 | 71.80 | 공백무시 |
| 20 | PaddleOCR-VL | 95.60 | 66.20 | 48.90 | 70.20 | 정식 |
| 21 | DeepSeek OCR | 95.80 | 64.50 | 46.60 | 69.00 | 정식 |
| 22 | olmOCR v0.2.0 | 95.20 | 65.00 | 44.90 | 68.40 | 정식 |
| 23 | GLM-4.1V-OCR | **97.40** | 52.90 | 30.00 | 60.10 | 정식 |

**최종 파이프라인** — `추론(본문 4MP) → 회전 보정 → edge60 → 안전HF(쪽번호) → lead`.
1위(@4MP 전체)와는 **0.01p** 차이인데 표가 58.34 vs 58.27 로 앞서서 이쪽을 채택했다.
...
#### 안전HF — 대상을 쪽번호로 좁힌 머리말 제거

머리말 후처리는 5가지가 전부 3축을 깎았다(§9). 그런데 **지우는 대상을 쪽번호로 좁히자**
처음으로 순증이 났다. 세 해상도에서 방향과 크기가 같다.

| 해상도 | Header/Footer | Long Text | Table | 3축 |
|---|---:|---:|---:|---:|
| 2MP → +안전HF | 95.96 → **96.09** | 76.41 → 76.39 | 58.32 → 58.32 | 76.90 → **76.93** |
| 4MP → +안전HF | 95.71 → **95.83** | 79.76 → 79.75 | 58.27 → 58.27 | 77.91 → **77.95** |
| 본문4MP → +안전HF | 95.71 → **95.83** | 79.67 → 79.65 | 58.34 → 58.34 | 77.91 → **77.94** |

머리말 **+0.12~0.13p**, 본문 **−0.01~0.02p**, 표 **불변**, 3축 **+0.03~0.04p**.
§11 의 교환비(머리말 1건 = 본문 8건)를 처음으로 넘긴 후처리다 — **지우는 범위를 좁힌 것**
말고는 이전 5종과 다르지 않다.

★ 폭이 작다. 판정은 여전히 점추정이고 부트스트랩 CI 는 재지 않았다(§10 결함 14).
...
| Header/Footer 는 후처리로 올릴 수 있다 | **5가지 전부 3축을 깎았다.** 평가셋 정답을 직접 보고 지우는 oracle 조차 +0.58p 다. 단 **대상을 쪽번호로 좁힌 변형(안전HF)만 +0.03~0.04p** 로 유일하게 순증(§8) |
